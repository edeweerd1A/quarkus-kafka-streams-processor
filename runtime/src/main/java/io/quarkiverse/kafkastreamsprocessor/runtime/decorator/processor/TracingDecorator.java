...
public class TracingDecorator extends AbstractProcessorDecorator {
    ...
    public void process(Record record) {
        ...
        try (
                Scope ignored = (extractedContext != null) ? Baggage.fromContext(extractedContext).makeCurrent() : Scope.noop();
    ...
    void logInputMessageMetadata(Record record) {
        if (log.isDebugEnabled()) {
            ...
            LoggedRecord logRecord = new LoggedRecord(
                    marshallValue(record),
                    headers.get(KafkaStreamsProcessorHeaders.UUID),
                    ZonedDateTime.ofInstant(Instant.ofEpochMilli(record.timestamp()), ZoneOffset.UTC),
                    headers,
                    context.applicationId(),
                    extractTopic(),
                    extractPartition());
            log.debug("Input message is {}", logRecord);
        }
    }

    private String marshallValue(Record<?, ?> record) {
        if (record.value() instanceof MessageOrBuilder) {
            try {
                return jsonPrinter.print((MessageOrBuilder) record.value());
    ...
    private String extractTopic() {
        if (context.recordMetadata().isPresent()) {
            return context.recordMetadata().get().topic();
        }
        return null;
    }
    ...
    private int extractPartition() {
        if (context.recordMetadata().isPresent()) {
            return context.recordMetadata().get().partition();
        }
        return -1;
    }
